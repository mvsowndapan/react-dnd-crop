...
var validateImage = exports.validateImage = function validateImage(file, callback, cropRatio, freeCrop) {
  var _URL = window.URL || window.webkitURL;
  var imageRatio = cropRatio.aspect || 4 / 3;
  var img = new Image();
  if (file) {
    img = new Image();
    ...
    img.onload = function () {
      ...
      reader.onload = function () {
        if (imageRatio === _this.width / _this.height || freeCrop === true || !cropRatio.aspect) return callback(true, reader.result);else return callback(false, reader.result);
      };
    };
  }
};
